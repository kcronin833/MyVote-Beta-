-- MyVote Full Database Schema (clean run)
...
DROP POLICY IF EXISTS "viewpoint_likes_select_all" ON public.viewpoint_likes;
DROP POLICY IF EXISTS "viewpoint_likes_insert_own" ON public.viewpoint_likes;
DROP POLICY IF EXISTS "viewpoint_likes_delete_own" ON public.viewpoint_likes;
...
DROP POLICY IF EXISTS "pol_profiles_select_own" ON public.political_profiles;
DROP POLICY IF EXISTS "pol_profiles_insert_own" ON public.political_profiles;
DROP POLICY IF EXISTS "pol_profiles_update_own" ON public.political_profiles;
...
DROP POLICY IF EXISTS "suggestion_votes_select_all" ON public.suggestion_votes;
...
DROP POLICY IF EXISTS "merch_select_approved" ON public.merch_products;
DROP POLICY IF EXISTS "merch_insert_auth" ON public.merch_products;
DROP POLICY IF EXISTS "merch_update_own" ON public.merch_products;
...
CREATE POLICY "merch_update_own" ON public.merch_products FOR UPDATE USING (
  auth.uid() = submitted_by OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true)
);

-- 12. Triggers for likes count
...
-- 13. Recalculate political score
...
DECLARE
  ...
  new_score NUMERIC;
BEGIN
  SELECT COUNT(*) FILTER (WHERE viewpoint_side = 'left'),
  ...
  UPDATE public.profiles SET political_score = new_score, updated_at = NOW()
...
CREATE TRIGGER on_viewpoint_like_change
  ...
  FOR EACH ROW EXECUTE FUNCTION public.recalculate_political_score();
...
CREATE INDEX IF NOT EXISTS idx_viewpoint_likes_user_id ON public.viewpoint_likes(user_id);
