      ...
      <div className="w-full aspect-video rounded-xl overflow-hidden mb-2 relative bg-muted">
        {article.image_url && !imgError ? (
          ...
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${accent}cc 0%, ${accent}33 100%)`,
            }}
          >
    ...
    <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
      <div className="p-4 pb-3 space-y-2">
        ...
        <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
          {spread >= 4 && (
            <span className="bg-teal-100 text-teal-700 font-semibold px-2 py-0.5 rounded-full">
              Cross-spectrum
            </span>
          )}
          {spread >= 2 && spread < 4 && (
        ...
        <div className="flex items-center gap-3 pt-0.5">
          {leftCount > 0 && (
            <span className="text-[11px] font-semibold text-blue-600">{leftCount} Left</span>
          )}
          {centerCount > 0 && (
            <span className="text-[11px] font-semibold text-slate-500">{centerCount} Center</span>
          )}
          {rightCount > 0 && (
            <span className="text-[11px] font-semibold text-red-600">{rightCount} Right</span>
          )}
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border p-8 text-center space-y-2">
          ...
          <button
            onClick={() => load()}
            className="mt-2 px-4 py-1.5 bg-teal-600 text-white text-sm font-semibold rounded-xl hover:bg-teal-700"
          >
            Try again
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-8 text-center">
          <p className="text-sm text-muted-foreground">
            No stories yet — run the pipeline from the admin panel.
          </p>
        </div>
      ) : visibleStories.length === 0 ? (
        <div className="bg-white rounded-2xl border border-border p-8 text-center">
          <p className="text-sm text-muted-foreground">
            No {filter}-leaning articles found in current stories.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
